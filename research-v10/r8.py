...
columns = ['security','close','down_rate','up_rate','prev_0']
...
for trading_date in trading_dates:
    ...
    if len(position)>0:
        ...
        if len(rs)>0:
            rec = rs[columns].iloc[0]

            close = rec['close']
            change = rec['prev_0']
            up_rate = rec['up_rate']
            down_rate = rec['down_rate']

            up_rate_change = rec['up_rate'] - position.iloc[0]['up_rate']
    ...
    # 如果空仓就选股
    if len(position)==0:
        query = "(prev_0<2 & prev_0>-1) and (high!=low) and security!='{}' \
            and prev_1>0 \
            and down_rate < down_rate_ma \
            and down_rate < prev_down_rate_1 \
            and down_rate < prev_down_rate_2 ".format(security)
        subset = subset[subset.eval(query)]
        factors = ['down_rate','money','down_rate']
        rs_f1 = subset.sort_values(by=[factors[0]],ascending=False)[:40]
        rs_f2 = subset.sort_values(by=[factors[1]],ascending=True)[:40]
