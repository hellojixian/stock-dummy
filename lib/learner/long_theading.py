...
# N_THREAD = multiprocessing.cpu_count() /2
N_THREAD = 7
...
class Learner(object):
    ...
    def get_fitness(self, dna_series):         
        ...
        def runner(v, start, end,progress,lock):
            for i in range(start,end):
                ...
                print('\rEvaluating: '+str(round(progress['processed_count']/(len(v)-1)*100,2))+"%"+" of "+str(len(v))+" DNA samples" + (" "*6),end='')
                # lock.release()
            return

        for i in range(len(splited)):    
            end = start + len(splited[i])
            t = threading.Thread(target=runner, args=(v, start, end,progress,lock))
            start = end
